\begin{abstract}
  ...
  operators).  Yet, a stand-alone tool to compare the efficiency and effectiveness of these methods is
  not available. Since existing mutation testing tools are often complex and language-dependent, this paper presents
  a tool, called \mr, that enables the ``retrospective'' study of many mutant reduction methods using the data collected
  from a prior analysis of all mutants. Focusing on the mutation operators and the mutants that they produce, the
  presented tool allows developers to prototype and study mutant reduction methods without being burdened by
  the implementation details of mutation testing tools. In addition to describing the design and overviewing results
  from using \mr, this paper inaugurates the public release of this open-source tool.
