...
\begin{abstract}
    ...
    Due to the computational expense of computation analysis, it is
    ...
    This is known as a do \textit{fewer} or selective approach. I propose
    a tool, \textit{selectms}, that is able to compare a couple of the
    existing selective approaches for mutation analysis.
\end{abstract}
...
suites \cite{gopinath2015empirical}. Additionally, the quality of a database schema
can be determined through the use of mutation testing \cite{mcminn2015effectiveness}. In
this paper, I am using mutant data from performing mutation analysis on database schemas.
...
exactly one fault. The goal to have each modified, or faulty, version of the source code
...
considered to be \textit{equivalent} or the test cases are insufficient to kill it \cite{gopinath2015empirical}. An equivalent
...
The number of normal mutants can be calculated by summing the number of normal mutants that are both killed and alive, denoted $k$ and $a$, repectively.
...
mutant needs to be run against at least, possibly many, test cases, mutation
...
Unified random sampling is the simplest of the sampling approaches, proposed by Timothy A. Budd,
...
Wong et al. first suggested sampling the same number of mutants from each operator, know as
...
instead of applying the fraction over all mutants, it is applied across operators. In other
...
At this point, these are the only reduction strategies present in my research, as time
goes on and I implement more approaches, I will discuss them in further detail.

\section{Method of Approach}
For the analyzing the existing selective approaches discussed previously, I have created
...
and schema. Next, the dbms attribute is as the name suggests, a character value representing the database management system under observation.
...
A boolean value, killed, is either true---if the mutant is dead---or false---if the mutant is alive. Finally, the time attribute is an
...
contain 118860 mutants and takes 67287543 milliseconds to perform mutation
analysis on the entire set of mutants.
